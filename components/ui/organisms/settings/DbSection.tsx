...
export function DbSection({ d, isSuperAdmin, isAdmin, canActions = true }: DbSectionProps) {
  ...
  function handleExport() {
    ...
    startExport(async () => {
      const res = await exportProjectAction()
      if (!res.success) {
  ...
  function handleExportWithMedia() {
    ...
    startExportMedia(async () => {
      ...
      const a       = document.createElement('a')
      a.href        = url
      a.download    = res.data.filename.replace('.json', '') + '-media.zip'
      a.click()
      URL.revokeObjectURL(url)
    })
  }
  ...
  function handleFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    startImport(async () => {
      ...
      const res = await importProjectAction(parsed)
      if (!res.success) {
        toast.error(d.importError)
        return
      }
  ...
  return (
    <section className="space-y-8">
      ...
      {/* ── Block C: Purge images — admin + superAdmin, current project only */}
